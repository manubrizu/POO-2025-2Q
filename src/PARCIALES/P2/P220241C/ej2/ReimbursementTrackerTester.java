...
public class ReimbursementTrackerTester {
    public static void main(String[] args) {
        ...
        for (Map.Entry<YearMonth, Integer> samReimbursements : rT.getReimbursements("Sam", YearMonth.of(2024, 1), YearMonth.of(2024, 7))) {
            System.out.printf("Client Sam earned $%d in %s\n", samReimbursements.getValue(), samReimbursements.getKey());
        }
...
// Client Sam earned $4000 in 2024-06
        ...
        System.out.println(rT.getReimbursements("Ben", june2024, java.time.YearMonth.of(2025, 12)).isEmpty()); // true
        ...
        try {
            rT.addReimbursement("Adam", java.time.YearMonth.of(2000, 1), 1_000);
        } catch (Exception ex) {
            System.out.println(ex.getMessage()); // Client is not registered.
        }

        try {
            rT.getRemainingReimbursement("Adam", java.time.YearMonth.of(2000, 1));
        } catch (Exception ex) {
            System.out.println(ex.getMessage()); // Client is not registered.
        }

        try {
            rT.getReimbursements("Adam", java.time.YearMonth.of(2000, 1), java.time.YearMonth.of(2001, 1));
        } catch (Exception ex) {
            System.out.println(ex.getMessage()); // Client is not registered.
        }
    }
